# 🚀 Guia de Instalação - Sistema de Documentação Técnica
...
O sistema já está configurado para MongoDB local:
```
mongodb://127.0.0.1:27017/infra_docs
```

#### Opção B: MongoDB Remoto (Atlas ou outro)
...
### 1️⃣ O arquivo `app.js` já está configurado para acesso externo:
...
### 2️⃣ Liberar Porta no Firewall (Windows Server)
...
1. Abra `Windows Defender Firewall` > `Configurações Avançadas`
...
```json
{
  "tituloDocumento": "Documentação RACK001 - Físico",
  ...
  "secoes": [
    {
      ...
      "conteudo": {
        ...
        "detalhes": [
          {"rotulo": "Localização", "valor": "Datacenter Principal"},
    ...
    },
    {
      ...
      "conteudo": {
        "textoBruto": "Internet\n  |\n[pfSense] - 192.168.10.1\n  |\n[Switch UniFi]\n  |\n  ├─ SRVBASE01 - 192.168.10.201\n  └─ SRVDC01 - 192.168.10.200"
      }
    }
  ]
}
...
| Tipo | Uso | Campos |
|------|-----|--------|
| `infoGeral` | Informações gerais | `textoBruto`, `detalhes[]` |
| `credenciais` | Senhas e acessos | `textoBruto` |
| `mapaRede` | Diagramas ASCII | `textoBruto` |
| `blocoCodigo` | Código/comandos | `textoBruto` |
...
**💡 Dica**: O layout foi otimizado para impressão em A4 com margens de 2cm.
...
- Verifique se MongoDB está rodando: `net start MongoDB` (Windows)
- Teste conexão: `mongo --eval "db.version()"`

### Erro: "EADDRINUSE" (porta em uso)
- Altere a porta no `app.js`: `const PORT = 3000;`
- Ou finalize processo: `netstat -ano | findstr :1100`

### Página não carrega externamente
- Verifique firewall Windows
- Teste ping no servidor
- Confirme que HOST está como `0.0.0.0`
...
1. ✅ Importar documentação existente do PDF fornecido
2. ✅ Configurar backup automático
3. ✅ Configurar HTTPS (opcional)
4. ✅ Criar usuários com diferentes permissões (futuro)
5. ✅ Integrar com Active Directory (futuro)
